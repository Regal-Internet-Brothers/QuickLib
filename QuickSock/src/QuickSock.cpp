...
namespace quickLib
{
	namespace sockets
	{
		...
		namespace IO
		{
			...
			// Input:
			size_type readLengthOfString(QSocket* socket)
			{
				return (size_type)socket->read<uqshort>();
			}

			template <typename type> type rawRead(QSocket* socket)
			{
				type data;
			...
			template <> QSOCK_UCHAR rawRead<QSOCK_UCHAR>(QSocket* socket)
			{
			...
			template <typename type> type read(QSocket* socket)
			{
				return socket->rawRead<type>();
			}

			template <> QSOCK_UINT16 read<QSOCK_UINT16>(QSocket* socket) { return (socket->fixByteOrder) ? ntohs(socket->rawRead<QSOCK_UINT16>()) : socket->rawRead<QSOCK_UINT16>(); }
			...
			template <> QSOCK_UINT64 read<QSOCK_UINT64>(QSocket* socket) { return (socket->fixByteOrder) ? ntohll(socket->rawRead<QSOCK_UINT64>()) : socket->rawRead<QSOCK_UINT64>(); }

			template <> QSOCK_FLOAT32 read<QSOCK_FLOAT32>(QSocket* socket) { return (socket->fixByteOrder) ? ntohf(socket->rawRead<QSOCK_UINT32>()) : socket->rawRead<QSOCK_FLOAT32>(); }
			template <> QSOCK_FLOAT64 read<QSOCK_FLOAT64>(QSocket* socket) { return (socket->fixByteOrder) ? ntohd(socket->rawRead<QSOCK_UINT64>()) : socket->rawRead<QSOCK_FLOAT64>(); }

			template <> nativeString read<nativeString>(QSocket* socket)
			{
				// Read the string's length, then the string itself.
				return socket->readNativeString(readLengthOfString(socket));
			}

			template <> std::wstring read<std::wstring>(QSocket* socket)
			{
			...
			// Output:
			bool writeLengthOfString(QSocket* socket, size_type length)
			{
				return socket->write<uqshort>((uqshort)length);
			}

			template <typename type> bool rawWrite(QSocket* socket, type data)
			{
				return socket->writeData(&data, sizeof(type));
			}

			template <> bool rawWrite<QSOCK_UCHAR>(QSocket* socket, QSOCK_UCHAR data)
			{
				#ifndef QSOCK_THROW_EXCEPTIONS
			...
			template <> bool write<QSOCK_UINT16>(QSocket* socket, QSOCK_UINT16 data) { return (socket->fixByteOrder) ? socket->rawWrite<QSOCK_UINT16>(htons(data)) : socket->rawWrite<QSOCK_UINT16>(data); }
			template <> bool write<QSOCK_INT16>(QSocket* socket, QSOCK_INT16 data) { return write<QSOCK_UINT16>(socket, (QSOCK_UINT16)data); }

			template <> bool write<QSOCK_UINT32_LONG>(QSocket* socket, QSOCK_UINT32_LONG data)
			{
				return ((socket->fixByteOrder) ? socket->rawWrite<QSOCK_UINT32_LONG>(htonl(data)) : socket->rawWrite<QSOCK_UINT32_LONG>(data));
			}

			template <> bool write<QSOCK_UINT64>(QSocket* socket, QSOCK_UINT64 data)
			{
				return socket->rawWrite<QSOCK_UINT64>((socket->fixByteOrder) ? htonll(data) : data);
			}

			template <> bool write<QSOCK_FLOAT32>(QSocket* socket, QSOCK_FLOAT32 data) { return (socket->fixByteOrder) ? write<QSOCK_UINT32>(socket, htonf(data)) : rawWrite<QSOCK_FLOAT32>(socket, data); }
			template <> bool write<QSOCK_FLOAT64>(QSocket* socket, QSOCK_FLOAT64 data) { return (socket->fixByteOrder) ? write<QSOCK_UINT64>(socket, htond(data)) : rawWrite<QSOCK_FLOAT64>(socket, data); }

			template <> bool write<nativeString>(QSocket* socket, nativeString str)
			{
				return socket->write(str, lengthOfString(str));
			}

			template <> bool write<std::wstring>(QSocket* socket, std::wstring wstr)
			{
		...
		QSOCK_INT32 QSocket::outputMessage(bool resetLength)
		{
			...
			QSOCK_INT32 transferred = sendto(_socket, (const QSOCK_CHAR*)outbuffer, (int)outbufferlen, 0, (const sockaddr*)&so_Destination, sizeof(sockaddr)); // No flags added for now.
		...
		bool QSocket::padBytes(size_type amount)
		{
			if (amount == 0)
			{
				if (writeOffset == _bufferlen)
					return false;

				amount = writeOffset-_bufferlen;
			}
